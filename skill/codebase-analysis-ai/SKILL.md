...
11. Never overwrite existing hooks or workflows. Agent instructions preserve all manual content; create the selected agent file when absent and refresh only the managed block when it is already present. Stop on an incomplete managed block or an unrecognized hook or workflow conflict.
