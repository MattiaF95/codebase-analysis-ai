...
11. Never overwrite unmanaged runtime, hooks, workflows, or agent instructions. After setup confirmation, refresh outdated managed runtime, hooks, workflows, and managed agent blocks while preserving all unrelated manual content. Stop on an incomplete managed block or an unrecognized conflict.
12. Do not commit, push, merge, or enable branch protection unless the user explicitly requests that external action.
13. Create persistent macro-area analyzer profiles only during `bootstrap`, after the parent agent has derived macro-areas from structural evidence and received the user's delegation decision. When at least two independent macro-areas each contain 15 or more relevant files, profile creation and delegation for those areas are mandatory. `setup`, `audit`, `update`, and `migrate` must not create or modify them.
...
6. Detect the active host and its delegation capability. In `bootstrap`, create one project-level, read-only analyzer profile for every approved macro-area when at least two independent macro-areas each contain 15 or more relevant files. This threshold is mandatory and must not be replaced by an agent judgment or disabled by `parent-only`. Below the threshold, use `parent-only` unless the user explicitly selects another supported policy. Create missing profiles; reuse managed profiles; never overwrite an unmanaged profile. In `update` and `audit`, reuse only existing profiles for impacted or selected areas; do not create persistent profiles. Before reuse, verify the profile's managed marker, host syntax, explicit read-only restrictions, and allowed paths; treat missing paths, unresolved placeholders, or changed area boundaries as stale. Render the area's allowed paths into every invocation brief. Never generate profiles for inactive hosts or write user-level profiles. A missing profile during bootstrap triggers a parent fallback only after the mandatory creation attempt fails and the concrete cause is recorded.
...
11. Validate names, links, source mappings, hashes, managed sections, report coverage, findings coverage, truncation handling, and the absence of analyzer writes before completion.
