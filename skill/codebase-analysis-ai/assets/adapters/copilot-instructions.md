...
2. Run `python3 <installed-skill-root>/scripts/codebase_analysis_ai.py --root . setup-state --agents copilot`, then, only when the runtime is current, run `python3 tools/codebase-analysis-ai/check.py check --mode working-tree` (`py -3` on Windows).
