...
1. Run `python tools/codebase-analysis-ai/check.py setup-state --agents gemini`, then `python tools/codebase-analysis-ai/check.py check --mode working-tree`.
2. If no documentation is impacted, do not activate the skill.
3. If documentation is stale, activate `codebase-analysis-ai` in incremental update mode.
