...
After `setup-state`, always include automation in the single initial contract. For the runtime, each active-host adapter, every required hook, and the GitHub Action, report one proposed action: `create`, `refresh`, `repair permissions`, `keep`, or `resolve conflict`. Missing components must be proposed as `create`, outdated managed components as `refresh`, inactive managed hooks as `repair permissions`, current managed components as `keep`, and unmanaged collisions or unexpected installed runtime files as `resolve conflict`. An `unverified` runtime means `setup-state` was run from the project runtime itself; rerun it with the current installed skill before proposing setup changes. Briefly explain the operational effect and ask for confirmation once for the complete contract.
