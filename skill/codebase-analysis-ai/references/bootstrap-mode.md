...
7. Create `docs/_meta/documentation-map.json`, `state.json`, and `docs/_meta/coverage.md`. Register `docs/index.md` as `documentation.index` without source patterns or source hashes, persist the approved taxonomy and area source mappings, then add direct related-document links with one-level traversal. Exclude `docs/_archive/` from active metadata.
8. Validate language consistency, readability, acronym definitions, links, naming, map coverage, index completeness, source evidence, archive exclusion, secret exclusions, profile syntax, and absence of analyzer writes. Confirm that `docs/index.md` links to coverage with `_meta/coverage.md`, then run link validation and `validate-docs`. Add `--strict` only when the repository has explicitly adopted the managed document contract.
