...
- Preserve the scope markers supplied by managed templates and validate changed documents with `python3 tools/codebase-analysis-ai/check.py validate-docs <paths>` (`py -3` on Windows). Treat its default result as a migration warning; use `--strict` only when the repository has explicitly adopted the managed document contract. Pass only repository-relative document paths; the validator rejects paths that resolve outside the Git root and must not inspect external Markdown files.
