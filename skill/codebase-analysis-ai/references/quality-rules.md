...
- Preserve the scope markers supplied by managed templates and validate changed documents with `python tools/codebase-analysis-ai/check.py validate-docs <paths>`. Treat its default result as a migration warning; use `--strict` only when the repository has explicitly adopted the managed document contract.
