...
5. After confirmation, always run this exact command from the target repository root:
   ...
   The command creates only missing skill components. It never refreshes or replaces an existing agent file, hook, workflow, or unrelated automation.
...
8. Report every created, retained, or conflicted component and any local-only Git configuration. Do not classify agent rule files or installed automation as project documentation.

Setup must be idempotent. Re-running it must preserve existing managed blocks without duplication and preserve unrelated instructions.
